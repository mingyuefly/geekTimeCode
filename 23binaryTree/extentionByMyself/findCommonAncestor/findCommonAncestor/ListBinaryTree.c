...
/**
 寻找公共祖先
 */
Tnode* lowestCommonAncestor(Tree* root, Tnode* p, Tnode* q) {
    if(root == p ||root == q || !root)return root;
    
    Tnode* left = lowestCommonAncestor(root->leftChild,  p, q);
    Tnode* right = lowestCommonAncestor(root->rightChild,  p, q);
...
Tnode* lowestCommonAncestorData(Tree* root, int p, int q) {
    if(root->data == p ||root->data == q || !root)return root;
    
    Tnode* left = lowestCommonAncestorData(root->leftChild,  p, q);
    Tnode* right = lowestCommonAncestorData(root->rightChild,  p, q);
