...
public:
    ...
    LazyPrimMST(EdgeWeightGraph * g) {
        ...
        while (!pq->isEmpty()) {
            ...
            if (marked->at(v) == true && marked->at(w) == true) {
                continue;
            }
            mst->push(e);
...
private:
    ...
    void visit(EdgeWeightGraph * g, int v) {
        ...
        for (int i = 0; i < g->adj->at(v)->size(); i++) {
            ...
            if (marked->at(e->getOther(v)) == false) {
                pq->insert(e);
            }
        }
    }
...
int main(int argc, const char * argv[]) {
    ...
    LazyPrimMST lMST = LazyPrimMST(g);
    while (lMST.edges()->empty() == false) {
        Edge * e = lMST.edges()->front();
