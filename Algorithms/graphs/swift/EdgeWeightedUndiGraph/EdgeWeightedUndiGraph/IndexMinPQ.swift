...
class IndexMinPQ {
    private var keys: [Double] = [Double]()//保存具体的对象
    ...
    init(maxSize: Int) {
        maxN = maxSize
        keys = [Double](repeating: Double.greatestFiniteMagnitude, count: maxN + 1)
    ...
    func insert(_ index: Int, _ key: Double) {
    ...
    func deleteMin() -> Int {
        ...
        let min = pq[1]
        exch(1, N)
        keys[pq[N]] = Double.greatestFiniteMagnitude
        qp[pq[N]] = -1
        pq[N] = -1
        N -= 1
        if N > 1 {
    ...
    func changeKey(_ index: Int, _ key: Double) {
        guard validateIndex(index) else {
            return
        }
        guard contains(index) else {
